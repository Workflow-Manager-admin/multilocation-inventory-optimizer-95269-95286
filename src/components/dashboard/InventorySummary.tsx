...
export default function InventorySummary({ data }: InventorySummaryProps) {
  ...
  return (
    <div className="mb-4">
      <h2 className="text-base font-semibold mb-2 text-[color:var(--text-primary)]">Inventory Overview</h2>
      
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
        {summaryCards.map((card) => (
          <div 
            key={card.id} 
            className={`bg-[color:var(--bg-component)] border-t-3 ${card.color} rounded-md p-2 shadow-sm`}
          >
            <div className="flex items-start justify-between">
              <div className={`${card.iconColor} p-1.5 rounded-md bg-[color:var(--bg-canvas)]`}>
                {card.icon}
              </div>

...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 left-0 z-30 h-full w-64 bg-[color:var(--bg-component)] border-r border-[color:var(--border-light)]
          transform transition-transform duration-300 ease-in-out
          ${isOpen ? 'translate-x-0' : '-translate-x-full'} 
          lg:relative lg:translate-x-0 lg:min-h-[calc(100vh-56px)] lg:z-0 overflow-y-auto
        `}
      >
        {/* Mobile close button */}
        <button
          className="lg:hidden absolute top-2 right-2 p-2 rounded-md text-[color:var(--text-secondary)] hover:bg-[color:var(--bg-canvas)] focus:outline-none"
          onClick={onClose}
        ...
        {/* Navigation */}
        <nav className="py-3 px-2 mt-3 space-y-0.5">
          {navItems.map((item) => (
            <Link
              key={item.id}
              href={item.path}

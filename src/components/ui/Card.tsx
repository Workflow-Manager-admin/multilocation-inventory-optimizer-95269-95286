  ...
  return (
    <div className={`bg-[color:var(--bg-component)] rounded-md shadow-sm border border-[color:var(--border-light)] overflow-hidden ${className}`}>
      {(title || subtitle || headerAction) && (
        <div className="px-3 py-2 border-b border-[color:var(--border-light)]">
          <div className="flex items-center justify-between">
            <div>
              {title && <h3 className="text-base leading-6 font-medium text-[color:var(--text-primary)]">{title}</h3>}
              {subtitle && <p className="mt-0.5 max-w-2xl text-xs text-[color:var(--text-secondary)]">{subtitle}</p>}
            </div>
            {headerAction && (
              <div className="ml-4">
                {headerAction}
      ...
      {footer && (
        <div className="px-3 py-2 border-t border-[color:var(--border-light)]">
          {footer}
        </div>
      )}
    </div>
